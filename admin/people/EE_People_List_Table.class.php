...
class EE_People_List_Table extends EE_Admin_List_Table {
	...
	function column_PER_lname($item) {
		...
		$name_link = EE_Registry::instance()->CAP->current_user_can( 'ee_edit_people', 'espresso_people_edit_people' ) ?  '<a href="'.$edit_lnk_url.'" title="' . __( 'Edit Person', 'event_espresso' ) . '">' . $item->lname() . '</a>' : $item->lname();
		return $name_link;
	...
	function column_PER_fname($item) {
		...
		$actions = array();
		// edit person link
		if ( EE_Registry::instance()->CAP->current_user_can( 'ee_edit_people', 'espresso_people_edit_people' ) ) {
			$edit_lnk_url = EE_Admin_Page::add_query_args_and_nonce( array( 'action'=>'edit_person', 'post'=>$item->ID() ), EEA_PEOPLE_ADDON_ADMIN_URL );
		...
		if ( $this->_view == 'published'  || $this->_view == 'all' || $this->_view == 'draft' ) {
			// trash person link
			if ( EE_Registry::instance()->CAP->current_user_can( 'ee_delete_people', 'espresso_people_trash_people' ) ) {
			...
			if ( EE_Registry::instance()->CAP->current_user_can( 'ee_delete_people', 'espresso_people_restore_people' ) ) {
				// restore person link
		...
		$edit_lnk_url = EE_Admin_Page::add_query_args_and_nonce( array( 'action'=>'edit_person', 'post'=>$item->ID() ), EEA_PEOPLE_ADDON_ADMIN_URL );
		$name_link = EE_Registry::instance()->CAP->current_user_can( 'ee_edit_people', 'espresso_people_edit_people' ) ?  '<a href="'.$edit_lnk_url.'" title="' . __( 'Edit Person', 'event_espresso' ) . '">' . $item->fname() . '</a>' : $item->fname();
